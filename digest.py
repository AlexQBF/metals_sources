...
AI_SYSTEM = (
    "Ты — редактор ежедневного Telegram-дайджеста «AU и AG. Главное за день» "
    "по рынку ЗОЛОТА и СЕРЕБРА. Твоя задача — выбрать ГЛАВНОЕ за день именно "
    "по золоту и серебру и подать это коротко и по существу. "
    "Тебе дают посты из отраслевых каналов за последние ~1,5 суток и список тем "
    "прошлых дайджестов. Сделай дайджест по правилам:\n\n"
    "ЖЁСТКИЙ ФИЛЬТР ТЕМЫ. Оставляй ТОЛЬКО золото и серебро: их цены, спрос и "
    "предложение, добыча, запасы, прогнозы, отчёты золотодобытчиков, решения "
    "регуляторов и ЦБ по золоту/серебру, сделки и проекты по этим металлам.\n"
    "СТРОГО ВЫБРАСЫВАЙ всё про платину, палладий, МПГ/PGM, медь, никель, сталь, "
    "уголь, нефть, алмазы и прочие металлы и темы — даже если они рядом с золотом. "
    "Если пост в основном про платину или палладий — выбрасывай его целиком.\n\n"
    "ДРУГИЕ ПРАВИЛА:\n"
    ...
    "- Отбери значимое, отсортируй от важного к менее важному. Проходное и рекламу убери.\n"
    "- Не больше 8 пунктов.\n\n"
    "ФОРМАТ (живой, как в хорошем Telegram-канале, на русском):\n"
    "- Каждый пункт — заголовок жирным: <b>Заголовок</b>. Без эмодзи в пунктах.\n"
    "- Под заголовком 1-2 ёмких предложения: что произошло и почему важно. "
    "Конкретика (цифры, цены, компании) приветствуется.\n"
    "- Между пунктами пустая строка.\n"
    "- Пиши деловым, но живым языком, без воды и канцелярита.\n"
    "- Только HTML-тег <b>. Никакого Markdown. Без вступления и заключения.\n\n"
...
def fetch_prices():
    ...
    try:
        ...
        lines = [f"\n<b>Биржевые котировки</b> ({d})"]
        if gold is not None:
            lines.append(f"Золото: {fmt(gold)} $/унц")
        if silver is not None:
            lines.append(f"Серебро: {fmt(silver)} $/унц")
        return "\n".join(lines)
    except Exception as e:
...
def main():
    ...
    header = f"🪙 <b>AU и AG. Главное за день</b>\n{date_ru}\n\n"
